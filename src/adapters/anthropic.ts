...
export function createAnthropicAdapter(provider: OcxProviderConfig, cacheRetention?: "none" | "short" | "long"): ProviderAdapter {
  ...
  return {
    ...
    async *parseStream(response: Response): AsyncGenerator<AdapterEvent> {
      ...
      // Held message_delta chunk. Anthropic SSE event ordering invariant is
      //   content_block_stop -> message_delta -> message_stop.
      // If the upstream signals done=true after a message_delta but before
      // content_block_stop (Bun.serve + ReadableStream close race), we still
      // need to flush it AFTER the content_block_stop yields, in correct order.
      let heldMessageDelta: Record<string, unknown> | null = null;
      let heldStopReasonYielded = false;
      ...
      try {
        while (true) {
          ...
          if (done) {
            // Drain residual SSE lines the upstream pushed after our last loop
            // iteration. Process event-by-event using the same dispatch logic as
            // the main path so message_delta + message_stop in the tail land in
            // the right order.
            if (buffer.length > 0) {
              ...
              for (const line of tailLines) {
                ...
                tailEventType = ""; // <-- CRITICAL: reset between events (was a bug)
                switch (evType) {
                  case "message_start": {
                  ...
                  case "message_delta": {
                    // Hold until content_block_stop is queued so SSE ordering
                    // holds: content_block_stop -> message_delta -> message_stop.
                    const usage = td.usage as Record<string, number> | undefined;
                    pendingUsage = mergeAnthropicUsage(pendingUsage, usage);
                    heldMessageDelta = td;
                    break;
                  }
                  case "message_stop": {
                    // Flush held message_delta's usage right before stop (already
                    // merged into pendingUsage in the case branch above).
              ...
              // If the upstream closed mid-frame after a message_delta but
              // before its content_block_stop, force-flush the held delta's
              // pendingUsage and ensure done is emitted so we never hang.
              if (heldMessageDelta !== null) {
          ...
          for (const line of lines) {
            ...
            switch (currentEventType || data.type) {
              ...
              case "content_block_stop": {
                ...
                currentBlockType = "";
                // Flush any held message_delta now that content_block_stop has
                // landed (preserves stop -> delta SSE ordering if upstream
                // delivered these in reverse).
                if (heldMessageDelta !== null) {
                  const usage = heldMessageDelta.usage as Record<string, number> | undefined;
                  pendingUsage = mergeAnthropicUsage(pendingUsage, usage);
                  heldMessageDelta = null;
              ...
              case "message_stop": {
                // Flush any held message_delta before emitting done so its
                // pendingUsage is preserved precisely.
                if (heldMessageDelta !== null) {
                  const usage = heldMessageDelta.usage as Record<string, number> | undefined;
                  pendingUsage = mergeAnthropicUsage(pendingUsage, usage);
                  heldMessageDelta = null;
